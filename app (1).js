...
$('#helpBtn').onclick=()=>{$('#helpModal').classList.remove('hidden');renderHelp()};
...
splitNames=function(){return $('#nameBox').value.split(/\r?\n/).map(x=>x.trim()).filter(Boolean).map(x=>x.startsWith('[')&&x.endsWith(']')?x.slice(1,-1):x)};

$('#approvedInfo').onclick=e=>{e.preventDefault();$('#charsModal').classList.remove('hidden')};
